...
const [classType, setClassType] = useState("");
  ...
  value={firstName}
  onChange={(e) => setFirstName(e.target.value.replace(/[^A-Za-z' -]/g, ''))} // Filter non-letters
  pattern="^[A-Za-z]+(?:[' -][A-Za-z]+)*$" // HTML5 validation
  maxLength={20}
  required
...
<input
  ...
  value={lastName}
  onChange={(e) => setLastName(e.target.value.replace(/[^A-Za-z' -]/g, ''))} // Filter non-letters
  pattern="^[A-Za-z]+(?:[' -][A-Za-z]+)*$" // HTML5 validation
  maxLength={20}
  required
  className="w-full bg-white border border-gray-300 rounded-md py-2 px-3 text-base"
